...
def plot(x, y, z, contour=True):
    ...
    if xis1D and yis1D:
        ...
        return plot1D(y, np.transpose(z)[0], "$y$ [mm]", "z [µm]")
    elif yis1D and not xis1D:
        return plot1D(x, z[0], "$x$ [mm]", "$z$ [µm]")
    else:
        return plot2D(x, y, z)

def plot_profile(x, y, z, src, dst, *args, **kwargs):
    xextent = x[-1] - x[0]
    yextent = y[-1] - y[0]
    src_pixel = [0, 0]
    dst_pixel = [0, 0]
    ...
    z_profile = profile_line(z, src_pixel, dst_pixel, *args, **kwargs)
...
def plot2D(x, y, z, contour=False): 
    ...
    profile = ProfileBuilder(fig, ax, x, y, z)
    profile.connect()
    return fig
...
class ProfileBuilder(object):
    ...
    def on_release(self, event):
        ...
        if event.xdata != None and event.ydata != None:
            ...
            plot_profile(self.x, self.y, self.z, self.src, self.dst)
